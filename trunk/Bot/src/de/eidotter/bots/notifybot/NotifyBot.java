...
public class NotifyBot implements Observer {
	...
	public static final String BOT_HELP_SYNTAX = BOT_COMMAND + " <Nick>{,<Nick>} <Nachricht>";
	public static final String BOT_HELP = "Schickt eine Nachricht an die angegebene Nicks, sobald diese wieder auf den Server kommen.";
	...
	public void updateOnPrivateMessage(String sender, String login,
		...
		if(message.toLowerCase().startsWith("-tell ")){
			...
			if(parts.length > 2){
				...
				this.storeMessage(recipients, rec_message, sender);
			} else {
				// Falsches Format
			}
		}
	...
	private void storeMessage(String[] recipients, String rec_message, String sender) {
